...
class Schema
{
    ...
    public function getTableSchema()
    {
        ...
        $tableSchema = $allTableSchema[$this->modelName];
        if (method_exists($tableSchema['modelClass'], 'relations')) {
            $customRelations = $tableSchema['modelClass']::relations();
            $tableSchema['relations'] = array_merge($tableSchema['relations'], $customRelations);
//            var_dump($tableSchema['relations']);die;
        }
        return $tableSchema;
    }
    ...
    public function wrapValue($field, $value)
    {
        $wrapper = $this->getFieldValueWrapper($field);
    ...
    {
        $wrapper = "'";// $this->config->dbStringWrapper;
    ...
    {
        if (array_key_exists($action, $this->rights)) {
            $actionRights = $this->rights[$action];
    ...
    public function combineWhere()
    {
            ...
            if (is_array($condition)) {
                ...
                foreach ($condition as $key => $value) {
                    if (is_array($value)) {
                        ...
                        $conditionParts [] = $this->parseString($key) . " IS NULL";
                    } else {
        ...
        if (array_key_exists('#params', $conditions) && strlen(trim($conditions['#params'])) > 0) {
            if (strpos($where, '#params') === false) {
                $where .= " $and ( #params )";
                $paramsParts = preg_split('/[ ]/', $conditions['#params']);
                foreach ($paramsParts as &$paramsPart) {
                    if (array_key_exists($paramsPart, $conditions) || array_key_exists(lcfirst($paramsPart), $conditions)) {
                        $paramsPart = $conditions[lcfirst($paramsPart)];
                    } else {
                        $paramsPart = $this->parseString($paramsPart);
                    }
                }
                $conditions['#params'] = implode(' ', $paramsParts);
            }
        } else {
            if (strpos($where, '#params') !== false) {
...
//				}
    ...
    protected function collectSchemaParameters($schemaParameters)
    {
        if (isset($schemaParameters['where'])) {
            if (is_string($schemaParameters['where'])) {
                if ($this->checkBrackets($schemaParameters['where'])) {
                    $this->conditions['#params'] = $schemaParameters['where'];
                    foreach ($schemaParameters as $key => $value) {
                        if (strpos($key, "where") === 0 && $key !== "where") {
                            if (is_string($value)) {
                                if ($this->checkBrackets($value)) {
                                    $conditionName = lcfirst(substr($key, strlen('where')));
                                    $this->conditions['#' . $conditionName] = $value;
                                }
                            } else {
            ...
            if (is_array($schemaParameters['where'])) {
                $this->conditions['#params'] = $schemaParameters['where'];
            }
