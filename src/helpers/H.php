...
class H
{
    ...
    public static function toDbDate($date, $fromFormat = 'd.m.Y', $toFormat = 'Y-m-d')
    {
        if ($date == null) {
            return null;
        }
    ...
    public static function toDbDt($date, $fromFormat = 'd.m.Y H:i:s', $toFormat = 'Y-m-d H:i:s')
    {
        if ($date == null) {
            return null;
        }
    ...
    public static function fromDbDt($stringDate, $fromFormat = 'Y-m-d H:i:s')
    {
        return (\DateTime::createFromFormat($fromFormat, $stringDate));
    }
    ...
    public static function moneyFormat($amount, $currency = null)
    {
        $decimals = $amount % 100 === 0 ? 0 : 2;
        $number = number_format((float)($amount / 100), $decimals, '.', ' ');
        if ($currency === null) {
            return $number;
        }
    ...
    {
        $fio = trim(H::get($person, 'lname', '', '') . ' ' . mb_substr(H::get($person, 'fname', '', ''), 0,
